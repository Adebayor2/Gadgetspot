import React from 'react';
import { FiEye, FiEyeOff } from 'react-icons/fi';
import { STRENGTH_LABELS, STRENGTH_COLORS } from './constants';

export const PasswordStrengthBar = ({ strength }) => (
  <div className='mt-2 flex items-center gap-2'>
    <div className='flex-1 h-1.5 bg-slate-200 rounded-full overflow-hidden'>
      <div
        className={`h-full ${STRENGTH_COLORS[strength]} transition-all duration-300`}
        style={{ width: `${(strength / 5) * 100}%` }}
      />
    </div>
    <span className='text-xs font-semibold text-slate-600'>{STRENGTH_LABELS[strength]}</span>
  </div>
);

export const inputClass = (error, hasRightPadding = false) =>
  `w-full pl-11 ${hasRightPadding ? 'pr-12' : 'pr-4'} py-3 border-2 rounded-xl ` +
  `focus:outline-none focus:ring-2 transition-all duration-200 font-medium ` +
  `text-slate-900 placeholder-slate-400 ` +
  (error
    ? 'border-red-400 focus:border-red-400 focus:ring-red-100'
    : 'border-slate-200 focus:border-sky-400 focus:ring-sky-100');

export const FormField = ({ label, error, children }) => (
  <div>
    <label className='block text-sm font-semibold text-slate-800 mb-2.5'>{label}</label>
    {children}
    {error?.message && <p className='text-red-500 text-xs font-semibold mt-1.5'>{error.message}</p>}
  </div>
);

export const TogglePasswordButton = ({ show, onToggle }) => (
  <button
    type='button'
    onClick={onToggle}
    className='absolute right-3.5 top-3.5 text-slate-400 hover:text-sky-500 transition-colors duration-200'
    aria-label={show ? 'Hide password' : 'Show password'}
  >
    {show ? <FiEye className='h-5 w-5' /> : <FiEyeOff className='h-5 w-5' />}
  </button>
);

const GoogleIcon = () => (
  <svg className='w-4 h-4' viewBox='0 0 24 24' aria-hidden='true'>
    <path fill='#4285F4' d='M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z' />
    <path fill='#34A853' d='M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z' />
    <path fill='#FBBC05' d='M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z' />
    <path fill='#EA4335' d='M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z' />
  </svg>
)

export const GoogleButton = ({ label, onClick }) => (
  <button
    onClick={onClick}
    className='w-full flex items-center justify-center gap-2 bg-slate-50 hover:bg-slate-100 text-slate-700 font-semibold py-2.5 px-4 rounded-xl border-2 border-slate-200 transition-all duration-200 cursor-pointer'
  >
    <GoogleIcon />
    {label}
  </button>
)